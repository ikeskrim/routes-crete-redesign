...
# Grade D "amber soft"  — C+, the Warm Editorial rollout: C's warmth pushed
#                          toward amber, highlights that roll off instead of
#                          clipping (a shoulder), gentler filmic contrast, a
#                          faint warm black lift, turquoise calmed and foliage
#                          tempered toward olive. Chosen by eye from five
#                          prototypes on ten representative frames
#                          (.hunt/cplus/grade-proto/, local).
#
# Output: public/images/graded/<grade>/<relative path>, max 2400px long edge.
# ---------------------------------------------------------------------------
param(
  [ValidateSet('A', 'B', 'C', 'D')] [string]$Grade = 'A',
  [string]$Only = '',
  [int]$MaxEdge = 2400,
  [int]$Quality = 82
)
...
# shoulder  : highlight roll-off width (0 = none). Above the knee (1 - shoulder)
#             values ease exponentially toward white instead of clipping; the
#             slope is 1 at the knee, so nothing below it moves. 0 for A/B/C,
#             which therefore reproduce byte-for-byte.
$P = if ($Grade -eq 'A') {
  ...
  # D "amber soft" (C+). Read against C:
  #   gain    R 1.03 -> 1.05, G 1.005 -> 0.998, B 0.995 -> 0.962
  #                             warmth leans red/amber, not yellow-green
  #   lift    0.014 -> 0.026 (R) / 0.014 (B)   a faint warm floor, filmic
  #   contrast 1.10 -> 1.06    gentler S
  #   shoulder 0    -> 0.18    skies and cloud keep texture instead of clipping
  #   sat/vibrance 1.02/0.30 -> 0.98/0.20      richer than B, calmer than C
  #   cyanBoost 0.22 -> 0.08   turquoise stays turquoise, not neon, beside
  #                            terracotta
  #   greenTemper 0.15 -> 0.26 foliage toward olive
  #   vignette 0.08 -> 0.11    a printed page's edge, barely there
  @{ lift = @(0.026, 0.019, 0.014); gain = @(1.05, 0.998, 0.962); gamma = @(0.925, 0.955, 0.99); sat = 0.98; vignette = 0.11; contrast = 1.06; vibrance = 0.20; cyanBoost = 0.08; goldBoost = 0.14; greenTemper = 0.26; shoulder = 0.18 }
}
...
function Build-Lut([double]$lift, [double]$gain, [double]$gamma, [double]$contrast, [double]$shoulder = 0.0) {
  ...
  for ($i = 0; $i -lt 256; $i++) {
    ...
    if ($shoulder -gt 0 -and $v -gt $knee) {
      $v = $knee + $shoulder * (1.0 - [math]::Exp(-($v - $knee) / $shoulder))   # highlight shoulder
    }
...
# A/B/C carry no shoulder key; $null casts to 0, which skips the term.
$lutR = Build-Lut $P.lift[0] $P.gain[0] $P.gamma[0] $P.contrast ([double]$P.shoulder)
$lutG = Build-Lut $P.lift[1] $P.gain[1] $P.gamma[1] $P.contrast ([double]$P.shoulder)
$lutB = Build-Lut $P.lift[2] $P.gain[2] $P.gamma[2] $P.contrast ([double]$P.shoulder)
