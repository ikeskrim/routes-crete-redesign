# MONUMENT-LICENCE — the Greek licence for photographs of monuments

**Status, 17 September 2026: prepared, not submitted, nothing paid.** The
client submits the application and pays the fee. Nothing here was sent to
anyone.

**The ruling (client, 2026-09-17):**
- Prepare the application for commercial internet use under art. 46 of
  ν. 4858/2021, for the Fortezza, the Venetian lighthouse and Preveli
  Monastery.
- The frames stay live meanwhile. `content/photo-credits.json` marks each one
  "licence application pending".

---

## Read this first

- **The law asks for the licence *before* publication.** Art. 46 par. 4Β:
  «Σε κάθε περίπτωση απαιτείται λήψη προηγούμενης άδειας για την παραγωγή ή
  αναπαραγωγή προς διάδοση στο κοινό απεικονίσεων μνημείων, με σκοπό το
  κέρδος.» ("In every case a prior licence is required to produce or
  reproduce depictions of monuments for public dissemination for profit.")
- **Publishing without one is an offence.** Art. 66 par. 1 punishes it with up
  to three years' imprisonment.
- **The frames are live by the client's decision.** That exposure is the
  client's. We recommend a short written opinion from a lawyer, and the
  draft below says truthfully that the photographs are already online.
- **The licence covers only State-owned monuments** (art. 46 par. 4Δ(α)).
  Ownership is unconfirmed for all three subjects (see "The monuments").
  The application therefore asks the Ephorate to confirm, for each one,
  whether a licence is needed at all.
- **A photographer's licence is a separate thing.** The Creative Commons
  licences in our ledger cover the *photographs*. The Ministry licence covers
  the *monuments* in them. Each is needed; neither replaces the other.

## The monuments

| Monument | Protection | Owner | Under art. 46? |
|---|---|---|---|
| **Fortezza of Rethymno** (Archaeological Cadastre 153874) | Declared: ΥΑ 89901/2722/23.12.1941 (ΦΕΚ Β' 227/1941) for "Φρούριον (Καστέλλι)" with its mosque; the mosque alone by ΒΔ 13.1.1938 (ΦΕΚ Α' 18/1938). Built 1573–1580, so also an ancient monument protected by law (art. 6 par. 4). | **Unconfirmed, with signs both ways.** The Cadastre records that the State expropriated the houses inside, but the Municipality of Rethymno lets venues inside and calls them municipal property (Municipal Council decision 182/2025). | **Treat as in scope.** Ask the Ephorate to confirm ownership, and whether the Municipality must also consent. |
| **Venetian harbour of Rethymno and its lighthouse** (Cadastre 153875) | The harbour ("Ο Ενετικός λιμήν") was declared by ΥΑ 16307/9.9.1965 (ΦΕΚ Β' 605/1965). No separate declaration of the lighthouse was found, and it is not on the Navy Lighthouse Service's list of 46 lighthouses declared as monuments. Its date (1838) comes from secondary sources only. | **Unconfirmed.** The candidates are the State, the Navy lighthouse service and the municipal port fund (Δημοτικό Λιμενικό Ταμείο Ρεθύμνου), which runs the harbour. | **Uncertain.** Photos framed on the harbour show a declared monument. Include them, and ask which service is competent and whether a licence is needed. |
| **Preveli Monastery**: the Πίσω Μονή (St John the Theologian, Cadastre 153830) and the Κάτω Μονή (153829), within site 165050 | Declared historic place and archaeological site, zones A1, A2 and B. ΥΑ Α/Φ.31/36852/2942/12.10.1973 (ΦΕΚ Β' 1242/1973) names "Περιοχή Μονής Πρέβελη, Κουρταλιώτικο Φαράγγι". | **Most likely the Monastery** (Stavropegic and Patriarchal, Diocese of Lampi and Sfakia). No source shows State ownership. | **Probably not.** Ask the Ephorate to confirm. **Separately, ask the Monastery for written consent.** Its only public contact is webmaster@preveli.org. |

On the Preveli names: the Cadastre's monument records swap the two
dedications. The draft follows the Ephorate and the Monastery, who call the
Πίσω Μονή after St John the Theologian.

## Annex A — the photographs

**Four photographs, all third-party, all live on 2026-09-17.** Four is within
the 1–20 band. The site serves each as a colour-graded web copy, at most
2400 px on the long edge, with no watermark.

| # | File (site) | Monument | What it shows | Where on the site | Caption on the site | Author · licence · source | Date taken · shot |
|---|---|---|---|---|---|---|---|
| 1 | `rethymno-fortezza.jpg` | Fortezza | The fortress walls above Rethymno in daylight | `/transfers/private-transfers-rethymno`, first place break | "The Venetian Fortezza above Rethymno" | Marc Ryckaert (Commons user MJJR) · CC BY 3.0 · [File:Rethymno Old Fort R01.jpg](https://commons.wikimedia.org/wiki/File:Rethymno_Old_Fort_R01.jpg) | [from the file page] · [amateur/professional: ask or read the page] |
| 2 | `rethymno-harbour-lighthouse.jpg` | Venetian harbour and lighthouse | The harbour at dusk, the lighthouse lit, the town and a snow-covered peak behind | `/transfers/private-transfers-rethymno`, the page hero (since 2026-09-17) | "The Venetian harbour of Rethymno, its lighthouse lit" | C messier · **CC BY-SA 4.0** · [File:Λιμάνι Ρεθύμνου 6495.jpg](https://commons.wikimedia.org/wiki/File:%CE%9B%CE%B9%CE%BC%CE%AC%CE%BD%CE%B9_%CE%A1%CE%B5%CE%B8%CF%8D%CE%BC%CE%BD%CE%BF%CF%85_6495.jpg) | EXIF 2019-02-02 (camera clock not verified) · uploaded as own work, camera Olympus E-PL5 |
| 3 | `rethymno-harbour-mountains.jpg` | Venetian harbour and lighthouse | The lighthouse and harbour in daylight, snowy mountains behind | Homepage map (Rethymno preview) and the transfer page's route map | "Rethymno harbour, with the snow-covered mountains behind it" | Jebulon · CC0 1.0 · [File:Harbour snowy mountains Rethymno Crete Greece.jpg](https://commons.wikimedia.org/wiki/File:Harbour_snowy_mountains_Rethymno_Crete_Greece.jpg) | [from the file page] · [ask] |
| 4 | `preveli-monastery.jpg` | Preveli Monastery | A courtyard of the monastery, church and cells | Homepage map (Preveli Monastery preview) and the Kourtaliotis tour's route map | "Preveli Monastery" | Benoît Prieur · CC0 1.0 · [File:Monastère de Preveli - 1.JPG](https://commons.wikimedia.org/wiki/File:Monast%C3%A8re_de_Preveli_-_1.JPG) | [from the file page] · [ask] |

**Not in the application, and why:**
- **`rethymno-harbour-dusk.jpg`** (the harbour at night). It was the transfer
  hero until 2026-09-17 and appears on no page now. Its graded file is still
  reachable at its own URL, linked from nowhere.
- **`gorge-saint-nicholas-aerial.jpg`** (the chapel of Saint Nicholas in
  Kourtaliotiko Gorge). The gorge lies in the area named by the 1973
  declaration, but the chapel is not one of the three monuments in the
  ruling, and it is most likely church property. **Ask the Ephorate in the
  same letter** if the client wants certainty.
- **`rethymno-old-town-lane.jpg`** (a lane in the old town). It is a
  townscape, not a photograph of a monument. ΥΑ 436630/2023 art. 2 par. 2
  exempts open-air urban views that do not focus on a monument.

## What the client does, in order

1. **Fill in the brackets** in the letter below: company details, the URL,
   and the date the photographs first went online. Complete the two unknown
   columns of Annex A from each file page.
   - **The URL.** Until the cutover (`CUTOVER.md`), the site is public only
     at https://routes-crete-redesign.vercel.app. After it, the site is at
     https://www.routescrete.gr. Name both if the letter goes before the
     cutover.
   - **The dates.** Photographs 1, 3 and 4 have been on the vercel.app
     deployment since August 2026, and the site history gives the exact
     day of each on request. Photograph 2 went up on 17 September 2026.
2. **Take legal advice** on two points: the exposure for photographs already
   online, and whether the draft's statement about the colour grade (point ε)
   is the right way to put it.
3. **Download and fill in the Ministry's internet-use form:**
   https://www.culture.gov.gr/DocLib/aitisi_diadiktio_GR.doc (the Mitos link
   with spaces in the filename returns 404).
   - **Tick:** applicant «Τουριστικό Γραφείο»; purpose «Εμπορικός»;
     «Δισδιάστατη/Στατική»; access «Δωρεάν»; duration «Έως πέντε έτη».
   - **Fill in:** the URL, the server location and the operator.
   - The form's printed declaration is outdated (fees to Τ.Α.Π., a credit
     line under ν. 3028/2002). The covering letter commits to the current
     rules instead.
4. **Attach:**
   - the form;
   - the letter;
   - Annex A;
   - digital copies of the four photographs and of their captions;
   - for each photograph, a copy of its licence (the file page and the
     licence deed);
   - an ID copy of the signatory.

   Whether the Ephorate accepts a Creative Commons licence as the
   «άδεια χρήσης» it asks for is one of the letter's questions.
5. **Submit** in one of two ways:
   - by email to **efareth@culture.gr**;
   - in person or by post at **Αρκαδίου 214, Ρέθυμνο**. The registry is
     open Mon–Fri 07:30–15:30; tel. +30 28310 23653, 58842, 29975.

   There is no online route. Check the Ephorate head's name on
   [the Ministry page](https://www.culture.gov.gr/el/ministry/SitePages/viewyphresia.aspx?iID=1707)
   before sending; it listed Παπαδοπούλου Ελένη on 2026-09-17.
6. **Write to Preveli Monastery separately** (webmaster@preveli.org), asking
   for written consent to show photograph 4 on the site.
7. **Wait for the decision.** A single competent service decides within 5
   working days. If another service is competent (for example, for the
   lighthouse, ΥΝΜΤΕ Κρήτης, ynmkr@culture.gr), the application goes to its
   General Directorate. Mitos gives 5 days to 2 months overall.
8. **Pay** the fee set in the decision to **ODAP**, into the account printed
   in the decision, quoting its protocol number.
   - Mitos lists ODAP's Bank of Greece IBAN
     GR22 0100 0240 0000 0000 0267 864. Use the account in the decision if it
     differs; ν. 5324/2026 reorganised ODAP.
   - Email the proof of payment and invoicing details (name, activity, ΑΦΜ,
     tax office, address, phone, email) to ODAP and to the Ephorate. Earlier
     decisions used atpap@odap.gr and teli@odap.gr.
9. **Send us the decision.** The site then carries its conditions (see
   "After the licence").

## The fee

**The band:** internet use for a commercial or advertising purpose, 1–20
photographs, for more than one year and up to five years.

**The amount:** **€250 plus VAT**, which is €310 at the standard 24%. For
comparison, one year alone costs €100, and 20–50 photographs over up to five
years cost €500.

**Basis:** ΚΥΑ ΥΠΠΟΤ/ΔΟΕΠΥ/ΤΟΠΥΝΣ/126463/28.12.2011 (ΦΕΚ Β' 3046/2011,
p. 43879), art. 5 par. 2: «για 1−20 φωτογραφίες: για ένα έτος 100 ευρώ, για
περισσότερα από ένα έτη και έως πέντε έτη 250 ευρώ.» The only amendment,
ΚΥΑ 12569/2012 (Β' 648), does not touch art. 5. Mitos and ODAP still apply
this band.

**Risks, so the letter asks:**
- **A much higher tier.** If the Ephorate treated the images as advertising
  material under art. 4 par. 8, the fee would be €1,500 *per depiction*. The
  letter asks it to confirm the internet band.
- **The VAT rate** on the payment date was not checked.
- **The 2011 decision itself.** No newer fee decision was found, and its
  continuity under ν. 4858/2021 rests on practice, not an explicit clause.

## Three things the letter says plainly

A letter to the Ministry must be true. Three statements differ from the
research's first draft on purpose:
1. **The colour grade.** Every photograph on the site carries the same
   colour grade and is cropped to its slot. The draft says so (point ε) and
   declares that the *monuments* are not altered. It does not claim the
   photographs are untouched.
2. **Already online.** The photographs are already on the site, so the draft
   says so. It asks for the use to be regularised, and offers to take them
   down until the licence is issued if the Ephorate asks. It promises
   payment on the decision, not "before publication".
3. **Photograph 2 is CC BY-SA 4.0.** That licence obliges us to offer the
   graded copy for download under the same licence, and /credits does. A
   downloadable copy sits awkwardly with the form's question about
   "protection against further use", so the draft discloses it and asks.
   - **If the Ephorate objects,** the fix is on our side: take photograph 2
     off the hero. Its predecessor is kept in the content as `heroImage_r9`.
   - **Pulling the download link while the photograph stays is not an
     option.** The photographer's licence requires it.

## The application — Ελληνικά

```text
[ΝΟΜΙΚΗ ΕΠΩΝΥΜΙΑ ΕΠΙΧΕΙΡΗΣΗΣ] – διακριτικός τίτλος «Routes Crete»
ΑΦΜ: [ΑΦΜ] – ΔΟΥ: [ΔΟΥ]
Έδρα: [Οδός, αριθμός, Τ.Κ., Πόλη]
Τηλ.: [τηλέφωνο] – E-mail: [e-mail]

[Τόπος], [ημερομηνία]

ΠΡΟΣ:
Υπουργείο Πολιτισμού
Εφορεία Αρχαιοτήτων Ρεθύμνου
Αρκαδίου 214, Ρέθυμνο
(efareth@culture.gr)

ΘΕΜΑ: Αίτηση για τη χορήγηση άδειας χρήσης (αναπαραγωγής και διάδοσης στο
κοινό) φωτογραφικών απεικονίσεων μνημείων στον διαδικτυακό τόπο της
επιχείρησης, για εμπορικό σκοπό, για χρονικό διάστημα πέντε (5) ετών

ΣΧΕΤ.:
1. Το άρθρο 46 παρ. 4 έως 5 του Κώδικα νομοθεσίας για την προστασία των
   αρχαιοτήτων και εν γένει της πολιτιστικής κληρονομιάς (ν. 4858/2021, Α΄ 220).
2. Η υπ' αριθμ. ΥΠΠΟ/436630/18.9.2023 υπουργική απόφαση «Όροι, προϋποθέσεις,
   όργανα και διαδικασία χορήγησης άδειας για την παραγωγή, αναπαραγωγή και
   διάδοση στο κοινό απεικονίσεων μνημείων που ανήκουν στο Ελληνικό Δημόσιο»
   (Β΄ 5591).
3. Η υπ' αριθμ. ΥΠΠΟΤ/ΔΟΕΠΥ/ΤΟΠΥΝΣ/126463/28.12.2011 κοινή υπουργική απόφαση
   (Β΄ 3046), όπως τροποποιήθηκε με την υπ' αριθμ.
   ΥΠΠΟΤ/ΔΟΕΠΥ/ΤΟΠΥΝΣ/12569/7.2.2012 κοινή υπουργική απόφαση (Β΄ 648).

[Αξιότιμη κυρία Προϊσταμένη / Αξιότιμε κύριε Προϊστάμενε],

Η επιχείρηση [νομική επωνυμία], με διακριτικό τίτλο «Routes Crete» και ΑΦΜ
[ΑΦΜ], εδρεύει στ[διεύθυνση έδρας] και εκπροσωπείται νόμιμα από
[ονοματεπώνυμο], [ιδιότητα]. Είναι ιδιωτικός διοργανωτής περιηγήσεων στο
Ρέθυμνο.

Με την παρούσα αιτούμαστε, σύμφωνα με τα ανωτέρω σχετικά, άδεια χρήσης των
τεσσάρων (4) φωτογραφιών του συνημμένου Πίνακα (Παράρτημα Α), δηλαδή άδεια
για την αναπαραγωγή και διάδοσή τους στο κοινό αποκλειστικά μέσω του
διαδικτυακού τόπου της επιχείρησής μας. Οι φωτογραφίες απεικονίζουν μνημεία
της χωρικής αρμοδιότητας της Υπηρεσίας σας, και συγκεκριμένα: το Φρούριο
Φορτέτζα Ρεθύμνου· το Ενετικό Λιμάνι (Μανδράκι) του Ρεθύμνου με τον φάρο
του· τη Μονή Πρέβελη (Πίσω Μονή Αγίου Ιωάννη Θεολόγου).

Στοιχεία της αιτούμενης χρήσης:
α. Μέσο: αποκλειστικά ο διαδικτυακός τόπος [https://www.routescrete.gr, σήμερα
   προσωρινά στη διεύθυνση https://routes-crete-redesign.vercel.app], με ελεύθερη
   πρόσβαση του κοινού, χωρίς συνδρομή ή χρέωση. Γλώσσα προβολής: αγγλικά.
   Φορέας διαχείρισης / πάροχος φιλοξενίας: [στοιχεία]. Τοποθεσία
   διακομιστή: [χώρα]. Δεν ζητείται χρήση σε έντυπο, φυλλάδιο, συσκευασία ή
   άλλο υλικό φορέα.
β. Σκοπός: εμπορικός, δηλαδή η παρουσίαση των υπηρεσιών περιήγησης της
   επιχείρησης στον ιστότοπό της.
γ. Αριθμός φωτογραφιών: τέσσερις (4).
δ. Διάρκεια: πέντε (5) έτη από την έκδοση της άδειας.
ε. Είδος απεικόνισης: στατικές, δισδιάστατες φωτογραφίες. Όλες οι
   φωτογραφίες του ιστοτόπου φέρουν την ίδια χρωματική επεξεργασία και
   περικόπτονται στις διαστάσεις της θέσης τους· η μορφή των μνημείων δεν
   αλλοιώνεται. Ανάλυση: έως 2.400 εικονοστοιχεία στη μεγαλύτερη πλευρά, σε
   συμπιεσμένα αρχεία για το διαδίκτυο, χωρίς υδατοσήμανση.
στ. Προέλευση: και οι τέσσερις φωτογραφίες είναι έργα τρίτων δημιουργών,
   διαθέσιμα με άδειες Creative Commons (CC BY 3.0, CC BY-SA 4.0, CC0 1.0).
   Το Παράρτημα Α αναφέρει για κάθε φωτογραφία τον δημιουργό, την άδεια, την
   πηγή, την ημερομηνία λήψης και, όπου είναι γνωστό, αν η λήψη ήταν
   ερασιτεχνική ή επαγγελματική. Επισυνάπτονται οι σελίδες των αρχείων και τα
   κείμενα των αδειών. Παρακαλούμε να μας ενημερώσετε αν οι άδειες αυτές
   γίνονται δεκτές ως άδειες χρήσης κατά το άρθρο 3 παρ. 4 της σχετ. 2.
ζ. Νέες λήψεις: δεν προβλέπεται η πραγματοποίηση νέων λήψεων στα μνημεία.
η. Γνωστοποιήσεις: μετά την έκδοση της άδειας, κάθε φωτογραφία θα
   συνοδεύεται από την ονομασία του μνημείου και την αναφορά «Υπουργείο
   Πολιτισμού», σύμφωνα με το άρθρο 3 παρ. 2 της σχετ. 2, καθώς και από όποια
   άλλη γνωστοποίηση ορίσει η απόφασή σας.

Σημειώνουμε ότι οι φωτογραφίες του Παραρτήματος Α εμφανίζονται ήδη στον
ιστότοπο, από [ημερομηνία] (η φωτογραφία α/α 2 από 17.9.2026). Με την
παρούσα ζητούμε την τακτοποίηση της χρήσης τους. Εφόσον η Υπηρεσία σας το
ζητήσει, θα τις αποσύρουμε έως την έκδοση της άδειας.

Η φωτογραφία α/α 2 διατίθεται από τον δημιουργό της με άδεια Creative
Commons BY-SA 4.0. Η άδεια αυτή υποχρεώνει να διατίθεται και η επεξεργασμένη
εκδοχή της με την ίδια άδεια, γι' αυτό ο ιστότοπος παρέχει σύνδεσμο λήψης του
αρχείου στη σελίδα των πιστώσεων. Παρακαλούμε να μας ενημερώσετε αν αυτό
είναι συμβατό με την αιτούμενη άδεια· σε αντίθετη περίπτωση θα αποσύρουμε τη
φωτογραφία αυτή.

Σύμφωνα με το άρθρο 5 παρ. 2 της σχετ. 3 (τέλη χρήσης απεικονίσεων στο
διαδίκτυο), για τη χρήση 1–20 φωτογραφιών με εμπορικό ή διαφημιστικό σκοπό
για περισσότερα από ένα και έως πέντε έτη προβλέπεται τέλος 250 ευρώ, πλέον
του αναλογούντος ΦΠΑ. Παρακαλούμε να μας επιβεβαιώσετε ότι εφαρμόζεται η
κλίμακα αυτή, δεδομένου ότι η χρήση αφορά αποκλειστικά ιστότοπο και όχι
έντυπο ή άλλο διαφημιστικό υλικό (άρθρο 4 παρ. 8 της σχετ. 3). Θα
καταβάλουμε το τέλος που θα ορίσει η απόφασή σας υπέρ του Οργανισμού
Διαχείρισης και Ανάπτυξης Πολιτιστικών Πόρων (Ο.Δ.Α.Π.) αμέσως μετά την
έκδοσή της, στον λογαριασμό και με την αιτιολογία που θα αναφέρει,
συμπεριλαμβανομένου του αριθμού πρωτοκόλλου της. Το αποδεικτικό πληρωμής και
τα στοιχεία τιμολόγησης θα αποσταλούν στον Ο.Δ.Α.Π. και στην Υπηρεσία σας.

Δηλώνουμε ότι:
(1) θα χρησιμοποιούμε τις φωτογραφίες μόνο για τον σκοπό, στο μέσο και για
    το χρονικό διάστημα της άδειας, και για κάθε νέα χρήση θα ζητήσουμε νέα
    έγκριση·
(2) η άδεια δεν θα μεταβιβαστεί σε τρίτους·
(3) δεν θα αλλοιώσουμε τη μορφή των μνημείων και οι απεικονίσεις δεν θα τα
    θίγουν·
(4) την ευθύνη για την ορθή και νόμιμη χρήση τους φέρει αποκλειστικά η
    επιχείρησή μας·
(5) η άδεια δεν συνιστά συνεργασία με το Υπουργείο Πολιτισμού·
(6) θα ενημερώνουμε την Υπηρεσία σας για τις ημερομηνίες ανάρτησης και
    απόσυρσης των φωτογραφιών.

Παρακαλούμε επίσης να μας ενημερώσετε:
(α) αν για κάποιο από τα μνημεία του Παραρτήματος Α, ιδίως για τον φάρο του
    Ενετικού Λιμανιού και για τη Μονή Πρέβελη, δεν απαιτείται άδεια κατά το
    άρθρο 46 παρ. 4 του ν. 4858/2021, επειδή δεν ανήκει στο Ελληνικό Δημόσιο·
(β) αν για κάποιο από αυτά, ιδίως για τον φάρο, εφόσον χρονολογείται μετά το
    1830, αρμόδια είναι άλλη Υπηρεσία του Υπουργείου Πολιτισμού (π.χ. η
    Υπηρεσία Νεωτέρων Μνημείων και Τεχνικών Έργων Κρήτης), ώστε να
    απευθυνθούμε αναλόγως·
(γ) αν, δεδομένου ότι χώροι εντός της Φορτέτζας παραχωρούνται από τον Δήμο
    Ρεθύμνης (απόφαση 182/2025 του Δημοτικού Συμβουλίου), απαιτείται και η
    σύμφωνη γνώμη του Δήμου.

Για διευκρινίσεις ή συμπληρωματικά στοιχεία μπορείτε να επικοινωνήσετε με
[ονοματεπώνυμο υπευθύνου], τηλ. [τηλέφωνο], e-mail [e-mail].

Συνημμένα:
1. Συμπληρωμένο έντυπο του Υπουργείου Πολιτισμού «Αίτηση για τη χορήγηση
   αδείας χρήσης απεικονίσεων αρχαιολογικού περιεχομένου ... στο διαδίκτυο».
2. Παράρτημα Α – Πίνακας φωτογραφιών (α/α, όνομα αρχείου, μνημείο,
   περιγραφή, λεζάντα, σελίδα/θέση στον ιστότοπο, δημιουργός, άδεια, πηγή,
   ημερομηνία λήψης, ερασιτεχνική/επαγγελματική λήψη).
3. Ψηφιακά αντίγραφα των τεσσάρων φωτογραφιών.
4. Ψηφιακό αντίγραφο των λεζαντών που συνοδεύουν τις φωτογραφίες.
5. Οι σελίδες των αρχείων και τα κείμενα των αδειών Creative Commons.
6. Αντίγραφο ταυτότητας του νόμιμου εκπροσώπου [και έγγραφο εκπροσώπησης της
   επιχείρησης, εφόσον ζητηθεί].

Για την επιχείρηση
[Ονοματεπώνυμο νόμιμου εκπροσώπου]
[Ιδιότητα]
[Υπογραφή – σφραγίδα]
```

## The application — English (a translation, for the client)

```text
[LEGAL NAME OF THE BUSINESS] – trading as "Routes Crete"
Tax no. (ΑΦΜ): [ΑΦΜ] – Tax office (ΔΟΥ): [ΔΟΥ]
Registered office: [street, number, postcode, city]
Tel.: [phone] – E-mail: [e-mail]

[Place], [date]

TO: Ministry of Culture, Ephorate of Antiquities of Rethymno,
Arkadiou 214, Rethymno (efareth@culture.gr)

SUBJECT: Application for a licence to use (reproduce and make available to
the public) photographic depictions of monuments on the company's website,
for a commercial purpose, for five (5) years

REF.:
1. Law 4858/2021 (Gazette A' 220), article 46 paras 4 to 5.
2. Ministerial Decision ΥΠΠΟ/436630/18.9.2023 (Gazette B' 5591).
3. Joint Ministerial Decision ΥΠΠΟΤ/ΔΟΕΠΥ/ΤΟΠΥΝΣ/126463/28.12.2011
   (Gazette B' 3046), as amended by 12569/7.2.2012 (Gazette B' 648).

[Dear Madam / Dear Sir],

[Legal name], trading as "Routes Crete", tax no. [ΑΦΜ], registered at
[address] and legally represented by [full name], [capacity], is a private
tour operator in Rethymno.

We apply for a licence to use the four (4) photographs in the attached table
(Annex A), that is, to reproduce them and make them available to the public
solely on our company's website. They depict monuments within your Service's
competence: the Fortezza of Rethymno; the Venetian (Mandraki) harbour of
Rethymno and its lighthouse; Preveli Monastery (the rear monastery of St John
the Theologian).

Details of the requested use:
a. Medium: only the website [https://www.routescrete.gr, currently served at
   https://routes-crete-redesign.vercel.app], free to the public. Display language:
   English. Operator / hosting provider: [details]. Server location:
   [country]. No print, leaflet, packaging or other physical use is requested.
b. Purpose: commercial, presenting the company's touring services on its
   website.
c. Number of photographs: four (4).
d. Duration: five (5) years from the issue of the licence.
e. Type: still, two-dimensional photographs. Every photograph on the site
   carries the same colour grade and is cropped to its slot; the appearance
   of the monuments is not altered. Resolution: at most 2,400 pixels on the
   long edge, as compressed web files, without a watermark.
f. Origin: all four are works of third-party photographers, available under
   Creative Commons licences (CC BY 3.0, CC BY-SA 4.0, CC0 1.0). Annex A
   gives each one's author, licence, source, date taken and, where known,
   whether the shot was amateur or professional. The file pages and licence
   texts are attached. Please tell us whether these licences are accepted as
   use licences under article 3 para. 4 of Ref. 2.
g. New shoots: none planned.
h. Credits: once the licence is issued, each photograph will carry the
   monument's name and «Υπουργείο Πολιτισμού» ("Ministry of Culture"), as
   article 3 para. 2 of Ref. 2 requires, and any other credit your decision
   sets.

The photographs in Annex A are already on the website, since [date]
(photograph no. 2 since 17.9.2026). With this application we ask for their
use to be regularised. If your Service asks, we will take them down until the
licence is issued.

Photograph no. 2 is offered by its author under Creative Commons BY-SA 4.0.
That licence requires the edited version to be offered under the same
licence, so the website's credits page links the file for download. Please
tell us whether this is compatible with the requested licence; if not, we
will take that photograph down.

Under article 5 para. 2 of Ref. 3, internet use of 1–20 photographs for a
commercial or advertising purpose for more than one and up to five years
carries a fee of EUR 250 plus VAT. Please confirm that this band applies,
since the use is website-only and involves no print or other advertising
material (article 4 para. 8 of Ref. 3). We will pay the fee set in your
decision to ODAP immediately after it is issued, into the account and with
the reference it states, including its protocol number, and send the proof
of payment and our invoicing details to ODAP and to your Service.

We declare that:
(1) we will use the photographs only for the licensed purpose, medium and
    period, and will request new approval for any new use;
(2) the licence will not be transferred to third parties;
(3) we will not alter the appearance of the monuments, and the depictions
    will not harm them;
(4) our company alone is responsible for their correct and lawful use;
(5) the licence does not constitute cooperation with the Ministry of Culture;
(6) we will inform your Service of the dates the photographs are published
    and taken down.

Please also tell us:
(a) whether, for any monument in Annex A (in particular the harbour
    lighthouse and Preveli Monastery), no licence under article 46 para. 4 is
    needed because it does not belong to the Greek State;
(b) whether another Service is competent for any of them (in particular the
    lighthouse, if it dates from after 1830, e.g. the Service of Newer
    Monuments and Technical Works of Crete);
(c) whether, since the Municipality of Rethymno lets venues inside the
    Fortezza (Municipal Council decision 182/2025), the Municipality's
    consent is also required.

Contact: [name], tel. [phone], e-mail [e-mail].

Attachments: 1. the Ministry's internet-use form; 2. Annex A; 3. digital
copies of the four photographs; 4. their captions; 5. the file pages and
Creative Commons licence texts; 6. an ID copy of the legal representative
[and proof of representation, if requested].

For the company, [name], [capacity], [signature – stamp]
```

## After the licence: what changes on the site

When the decision arrives, send it to us. Then:
1. **Captions:** each licensed photograph's caption gains the monument's name
   and «Υπουργείο Πολιτισμού» (ΥΑ 436630/2023 art. 3 par. 2), plus any
   wording the decision adds. That is a caption-tier change: the credit
   guard and copy-subset are extended to check it.
2. **The ledger:** `monument.status` changes from "licence application
   pending" to the decision's number and expiry date.
3. **Renewal:** the licence lasts five years at most, and renewal needs a
   new application.
4. **Refusals:** a photograph the decision refuses comes off the site.

A future photograph of these monuments, whether from a photographer or from
stock, needs the same licence, so add it to this application (the band holds
up to 20 photographs), or to a new one.

## Open questions

**For a lawyer:**
- **Photographs already online.** What is the exposure, and can the
  Ephorate license existing amateur photographs retrospectively?
- **Amendments.** Has art. 46 been amended since 2021? ν. 5021/2023,
  ν. 5271/2026 and ν. 5324/2026 were checked and leave paras 4–5 alone;
  other laws were not checked.
- **The fee decision.** Is the 2011 decision (issued under ν. 3028/2002)
  still valid? ν. 4858/2021 has no explicit transitional clause.
- **Preveli.** Is a monastery's property outside «Ελληνικό Δημόσιο» for
  art. 46?
- **The colour grade.** Is point ε the right way to describe it?

**For the Ephorate** (the letter asks most of these):
- **Ownership:** of the Fortezza, the lighthouse and Preveli.
- **Competence:** which service handles the lighthouse.
- **Municipal consent:** for the Fortezza.
- **The fee:** the internet band rather than art. 4 par. 8; how photographs
  are counted; the VAT rate on the payment date.
- **Licences and downloads:** whether Creative Commons licences are accepted
  as use licences, and whether the BY-SA download link is compatible.
- **The form:** the 2018 internet form, the 2024 ΔΠΑΝΣΜ form, or a letter.
  Whether «Εμπορικός» is the right purpose box.
- **Documents:** whether a company applicant needs proof of representation.

**Records the research could not read:**
- The listed-monuments register (listedmonuments.culture.gr) no longer
  resolves.
- The Preveli ΦΕΚs of 1967, 1980, 1994 and 1998 were not read.
- The old-town ΦΕΚs of 1991 and 1997 were not read.

## Sources (all read on 2026-09-17)

**Law and decisions:**
- **ν. 4858/2021,** ΦΕΚ Α' 220/2021:
  https://api.et.gr/apiLAW/1/2021/4858/pdf. Art. 2, 6, 7, 46 and 66.
- **ΥΑ ΥΠΠΟ/436630/18.9.2023,** ΦΕΚ Β' 5591:
  https://api.et.gr/apiFEK/2/2023/5591/pdf
- **ΚΥΑ 126463/2011,** ΦΕΚ Β' 3046: https://api.et.gr/apiFEK/2/2011/3046/pdf.
  Its amendment, ΦΕΚ Β' 648/2012: https://api.et.gr/apiFEK/2/2012/648/pdf

**The procedure:**
- **Mitos, internet use of images of archaeological content:**
  https://mitos.gov.gr/index.php/ΔΔ:Άδεια_χρήσης_εικόνων_αρχαιολογικού_περιεχομένου_στο_διαδίκτυο
- **The Ministry's internet-use form:**
  https://www.culture.gov.gr/DocLib/aitisi_diadiktio_GR.doc
- **Ephorate of Antiquities of Rethymno:**
  https://www.culture.gov.gr/el/ministry/SitePages/viewyphresia.aspx?iID=1707
- **ΥΝΜΤΕ Κρήτης:**
  https://www.culture.gov.gr/el/ministry/SitePages/viewyphresia.aspx?iID=1381

**The monuments:**
- **Archaeological Cadastre records:**
  - Fortezza:
    https://ops.arxaiologikoktimatologio.gov.gr/api/portal/monument?id=153874
  - Harbour: https://ops.arxaiologikoktimatologio.gov.gr/api/portal/monument?id=153875
  - Preveli site:
    https://ops.arxaiologikoktimatologio.gov.gr/api/portal/space?id=165050
- **Declarations:**
  - ΦΕΚ Α' 18/1938: https://api.et.gr/apiFEK/1/1938/18/pdf
  - ΦΕΚ Β' 227/1941: https://api.et.gr/apiFEK/2/1941/227/pdf
  - ΦΕΚ Β' 605/1965: https://api.et.gr/apiFEK/2/1965/605/pdf
  - ΦΕΚ Β' 1242/1973: https://api.et.gr/apiFEK/2/1973/1242/pdf
- **Municipality of Rethymno, decision 182/2025:**
  https://www.rethymno.gr/files/items/1/14329/182,25_apofasi_d.s_orismos_ypsoys_misthomatos.pdf
- **Navy Lighthouse Service, lighthouses declared as monuments:**
  https://yf.hellenicnavy.gr/diatiritea-mnimeia/
- **Preveli Monastery:** https://preveli.org/files/moni/index.html

The saved copies and the full research record are local, in the gitignored
`.hunt/lock/` folder (monument/, monument-verify/, rulings/).
